...
* **Service Layer (`src/src/services/`)**: Orchestrates core business workflows—retrieves/creates sessions, fetches recent message context limits, feeds prompt builders, triggers the LLM Service, and invokes DB writes.
...
* **Repository Layer (`src/repositories/`)**: Wraps Prisma Client operations, separating database queries and CRUD functions from business rules.
